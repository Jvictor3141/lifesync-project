...
const TaskSection = ({
    ...
    const renderTasks = (periodTasks, period) => {
        ...
        return periodTasks.map((task) => (
            <div
                key={task.id}
                className="flex items-center justify-between p-3 rounded-lg border-[1px] border-gray-500 transition-all duration-300 hover:shadow-md"
                style={{ borderLeft: `3px solid ${task.cor}` }}
            >
                <Checkbox
                ...
                />
                <span
                    className={`flex-1 text-center text-white-700 transition-all duration-300 ${
                        task.completed ? "line-through opacity-60" : ""
                    }`}
                >
                    {task.text}
                </span>
                <span className="text-xs font-bold text-white-600 ml-3 min-w-[48px] text-right flex items-center">
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                                onClick={() => onRemoveTask(period, task.id)}
                            >
                                Remover
                            </AlertDialogAction>
                        </AlertDialogFooter>
    ...
    return (
        <div className="mt-8 glassmorphism rounded-xl shadow-md border p-6">
            ...
            <div className="flex flex-col md:flex-row gap-2 mb-4 justify-center items-center w-full">
                <Input
                    ...
                    className="flex-1 min-w-[220px] md:min-w-[320px] text-gray-200 max-w-md px-4 py-2 text-base"
                />
                <Input
                    type="time"
                    value={newTaskTime}
                    onChange={(e) => setNewTaskTime(e.target.value)}
                    className="w-[90px] md:w-[110px] px-2 py-1 text-sm text-gray-200 cursor-pointer"
                />
                ...
                <div className="flex items-center gap-2 ml-auto">
                    <label className="flex items-center gap-1 font-medium text-gray-200 text-sm">
                        <Checkbox
            ...
            <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-3">
                {periods.map((period) => (
                    <Card key={period.id} className="bg-gray-700">
